...
const Select: React.FC<SelectProps> = ({ question, handleChange }) => {
    ...
    switch (entity) {
        case Entities.singleSelectImage: {
            ...
            return <SingleSelectImage options={options} handleChange={handleChange} />;
        }
        case Entities.bubbleSelect: {
            const options = localeOptions.map((option) => ({ ...option }));
            return <BubbleSelect options={options} handleChange={handleChange} />;
        }
        case Entities.multipleSelect: {
            const options = localeOptions.map((option) => ({ ...option }));
            return <MultipleSelect options={options} handleChange={handleChange} />;
        }
        default: {
            const options = localeOptions.map((option) => ({ ...option }));
            return <SingleSelect handleChange={handleChange} options={options} />;
        }
    }
};
